...
| Locativo | INTERMEDIO | Solicitud, reserva, contrato, activación, finalización/cancelación y actos básicos existen; garantías, ajustes, rescisión contractual, renovación/extensión y Sync portable no tienen cierre runtime. |
...
| Gestión de conflictos Sync | SOPORTE_TRANSVERSAL | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-TEC-003` y la tabla preparatoria `conflicto_sincronizacion` definen detección, registro, clasificación, resolución y consulta; no existe un circuito application/API/tests integral. | Implementar el lifecycle funcional de conflictos, resolución autorizada, consultas y trazabilidad sin confundirlo con conflictos técnicos puntuales ya detectados por inbox/CAS. | OWNER_ISSUE = FALTA | Operaciones distribuidas; identidad portable | Sí para resolución operable de conflictos | ALTA |
| Gestión de jobs técnicos | SOPORTE_TRANSVERSAL | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-TEC-004` define registro, ejecución, reintento, cancelación y consulta de jobs; los entry points y estados técnicos puntuales existentes no materializan ese lifecycle general. | Implementar modelo/persistencia, application services, API/operación, tests y trazabilidad; el proceso scheduler productivo permanece separado. | OWNER_ISSUE = FALTA | Operaciones distribuidas; observabilidad | Sí para operación técnica administrable | ALTA |
| Importación/exportación técnica por lotes | SOPORTE_TRANSVERSAL | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-TEC-005` y estructuras preparatorias de paquetes definen generación, exportación, importación, procesamiento y consulta; no existe un circuito técnico integral. | Implementar formato/versionado, integridad, procesamiento idempotente, transporte, API/CLI, tests y trazabilidad sin confundirlo con el importador Excel de negocio. | OWNER_ISSUE = FALTA | Identidad portable; gestión de jobs | Sí para intercambio offline administrado | ALTA |
| Consulta técnica consolidada / diagnóstico técnico | SOPORTE_TRANSVERSAL | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-TEC-007` define una consulta read-only consolidada de operaciones distribuidas, Sync, conflictos, jobs, lotes técnicos, importación/exportación, respaldos, recuperaciones y trazabilidad; existen fuentes técnicas parciales, pero no el circuito consolidado. | Implementar query/application service, read model o repository de lectura, API/router técnico, tests, autorización aplicable e integración de las fuentes contratadas. No incluye monitoreo externo, dashboards de infraestructura, métricas, alertas ni mutación o reparación automática. | OWNER_ISSUE = FALTA | Fuentes de SRV-TEC-002..006; permisos de consulta | No para operación básica; sí para diagnóstico técnico integral | ALTA |
...
| Usuario: modificación | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-ADM-001` define la modificación general de Usuario; no existe un command runtime productivo para esa operación. | Implementar application service, API, persistencia, tests, seguridad y evento portable cuando corresponda. | COORDINADOR: #249; OWNER_ISSUE = FALTA focal | Seguridad y lifecycle portable | Sí para lifecycle completo | ALTA |
| Usuario: reactivación | NUCLEO_DOMINIO | PENDIENTE | No existe runtime ni contrato formal suficiente para reactivar un Usuario dado de baja. | Definir primero invariantes, autorización, credenciales/sesiones y efectos portables; luego implementar el circuito. | COORDINADOR: #249; OWNER_ISSUE = FALTA focal | Seguridad y lifecycle portable | Sí para lifecycle completo | ALTA |
...
| Bootstrap administrativo Usuario/Credencial | NUCLEO_DOMINIO | IMPLEMENTADO | El command local y `admin_credentials.py` permiten `init`/`reset` de credenciales administrativas, con tests focales. | Mantener el tooling administrativo separado del provisionamiento técnico de instalación, deployment y base. | COORDINADOR: #249 | Credenciales administrativas | No después del bootstrap inicial | ALTA |
...
| Autorizaciones administrativas explícitas | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-ADM-003` define solicitud, aprobación, rechazo, revocación y consulta de decisiones sobre operaciones sensibles; no existe circuito runtime específico. | Implementar persistencia, application services, API, tests y auditoría del workflow sin confundirlo con el enforcement de permisos GLOBAL/por Sucursal. | COORDINADOR: #249; OWNER_ISSUE = FALTA focal | Usuarios, roles/permisos y auditoría administrativa | No para autorización básica; sí para decisiones sensibles | MEDIA |
| Calendario comercial | NUCLEO_DOMINIO | IMPLEMENTADO | Singleton, programación, consulta y Sync portable; la configuración vigente se resuelve por intervalo y fecha efectiva en lectura. | No requiere command ni mutación futura para activar una configuración; la observabilidad del transporte Sync pertenece a infraestructura. | OWNERS_HISTORICOS_COMPLETADOS: #425/#486 | — | No | MEDIA |
...
| Auditoría administrativa | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-ADM-004` define registro, consolidación y consulta de eventos auditables; metadata CORE-EF e historiales técnicos puntuales no constituyen ese circuito. | Implementar persistencia, application services, API, tests y consulta administrativa especializada. | OWNER_VIGENTE: #265; COORDINADOR: #249 | Fuentes administrativas estables | No para operación básica | MEDIA |
| Consulta administrativa consolidada | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-ADM-006` define búsqueda, consulta histórica y reporte consolidado read-only de usuarios, roles, permisos, autorizaciones, auditoría y configuración; sólo existen consultas puntuales por recurso. | Implementar application/query service consolidado, read model o repository, API/router, tests, autorización e integración de fuentes sin confundirlo con Auditoría administrativa, frontend ni observabilidad. | OWNER_ISSUE = FALTA | Usuarios, roles/permisos, autorizaciones, auditoría y configuración | No para administración básica; sí para consulta integral | MEDIA |
| Frontend administrativo | NUCLEO_DOMINIO | PENDIENTE | UI principal no ofrece gestión administrativa integral. | Usuarios, roles, permisos, alcance/autorización, parámetros, calendario y auditoría; la gestión de Sucursal, Instalación y configuración local permanece bajo Operativo. | COORDINADOR: #249; OWNER_ISSUE = FALTA focal | Seguridad/contexto | Sí operación delegada | ALTA |
...
| Clasificaciones/condiciones auxiliares | SOPORTE_TRANSVERSAL | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-PER-004` define asignación, modificación, baja lógica y consulta de `persona_clasificacion`; no existe SQL ni runtime. El contrato la limita a soporte auxiliar o heredado. | Decidir catálogo y vigencias e implementar sólo el soporte transversal autorizado, sin absorber semántica de cliente, proveedor, titular o garante propia de otros dominios. | OWNER_ISSUE = FALTA | Persona base; decisión arquitectónica por clasificación | No para el núcleo de Personas | BAJA |
...
| Lifecycle | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Modificación mediante `PUT /api/v1/personas/{id_persona}` y baja lógica mediante `PATCH /api/v1/personas/{id_persona}/baja`, con services, persistencia, tests y metadata/`deleted_at`. | Completar propagación uniforme, lifecycle portable/Sync y restauración sólo si alguna vez se decide incorporarla. | OWNER_ISSUE = FALTA | Sync | Sí para ciclo completo | ALTA |
| Seguridad de Personas | SOPORTE_TRANSVERSAL | IMPLEMENTADO_PARCIALMENTE | Bearer, principal y helpers de autorización están disponibles; adopción desigual. | Migrar completamente los writes humanos y su scope. | OWNER_VIGENTE: #461 | #536 | Sí | CRÍTICA |
| Idempotencia de Personas | SOPORTE_TRANSVERSAL | IMPLEMENTADO_PARCIALMENTE | El ledger y los helpers comunes están disponibles; adopción desigual. | Adoptar idempotencia en los writes de Personas que correspondan. | OWNER_VIGENTE: #104 | Contrato CORE-EF | Sí en writes afectados | ALTA |
...
| Edificaciones | NUCLEO_DOMINIO | IMPLEMENTADO | CRUD, persistencia y tests, incluido XOR entre Inmueble y Unidad Funcional. | Cobertura UI y Sync portable. | OWNER_ISSUE = FALTA | Inmueble portable | No crítico | MEDIA |
| Servicios asociados al activo | SOPORTE_TRANSVERSAL | IMPLEMENTADO | CRUD de `servicio` y asociaciones con Inmueble/UF tienen SQL, API y tests. | Cobertura UI, portabilidad y eventos autoritativos cuando correspondan. | OWNER_ISSUE = FALTA | Inmueble/UF portable | No crítico | MEDIA |
| Facturas externas de servicios y responsables | NUCLEO_DOMINIO | IMPLEMENTADO | Registro/consulta de `factura_servicio`, asignación de responsables y materialización financiera explícita tienen SQL, API y tests. | Completar portabilidad y automatización event-driven sólo cuando exista contrato; los efectos de deuda permanecen bajo Financiero. | OWNER_ISSUE = FALTA | Servicios asociados; integración Financiero | Parcial multi-base | ALTA |
| Agrupaciones/relaciones estructurales | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | La asociación versionada Desarrollo → Inmueble tiene endpoints; no existe `relacion_inmobiliaria` ni agrupación compleja. | Completar sólo relaciones estructurales formalmente definidas, sin incorporar ownership de Persona, Comercial o Locativo. | OWNER_ISSUE = FALTA | Inmueble/Desarrollo | No para operación básica | MEDIA |
...
| Mejoras/infraestructura inmobiliaria especializada | NUCLEO_DOMINIO | PENDIENTE | Los nombres históricos de `SRV-INM-004/005` preservan estos conceptos sólo como roadmap; no existen contrato operativo completo, SQL ni runtime. | Definir modelo y lifecycle antes de incorporar una entidad propia, sin confundirla con Edificación, Servicio o Instalación operativa. | OWNER_ISSUE = FALTA | Inmueble/UF | No para núcleo actual | BAJA |
| Atributos/documentación inmobiliaria especializada | NUCLEO_DOMINIO | PENDIENTE | `SRV-INM-010` la conserva como roadmap conceptual; no existen contrato operativo completo, SQL, router, services ni tests. | Definir lifecycle y límite con Documental antes de implementar atributos especializados y referencias documentales del activo. | OWNER_ISSUE = FALTA | Inmueble; subsistema Documental | No para núcleo actual | BAJA |
...
| Lifecycle inmobiliario | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Metadata y commands puntuales. | Completar commands canónicos y ciclo de vida homogéneo. | OWNER_ISSUE = FALTA lifecycle focal | #536 en migraciones afectadas | Sí | ALTA |
| Seguridad inmobiliaria | SOPORTE_TRANSVERSAL | IMPLEMENTADO_PARCIALMENTE | Existen guards, Bearer y CAS en superficies puntuales. | Adoptar authz y CAS homogéneos. | OWNERS_VIGENTES de seguridad/evidencia: #461/#107 | Contexto #536 | Sí | ALTA |
...
| PPV2/cuotas | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Comercial posee el plan PPV2, sus bloques, la estructura contractual de cuotas y el preview/generación comercial. | Completar la regla/configuración pactada, base común, período objetivo y selector exacto; Financiero materializa obligaciones e indexación como integración dependiente, no como ownership Comercial. | OWNERS_VIGENTES por incremento: #427/#428/#429/#423 | Secuencia indicada; efectos financieros bajo Financiero | Sí para indexación definitiva | CRÍTICA |
...
| Configuración pactada de indexación | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Comercial gobierna el plan PPV2, sus bloques y la cláusula/regla contractual pactada. | Materializar para contratos nuevos la base común, el período objetivo y el selector exacto definidos por `INT-FIN-005`; consumir catálogo, valores, corridas, cálculo y aplicación gobernados por Financiero. | OWNERS_VIGENTES por alcance: #423/#349/#348/#374/#405 | #427–#429; capacidades de indexación de Financiero | Sí circuito avanzado | ALTA |
| Compatibilidad heredada PPV2 | COMPATIBILIDAD_HEREDADA | LEGACY_VIGENTE | Siguen materializados `fecha_base_indice` por bloque y el selector histórico equivalente a `fecha_valor <= fecha_objetivo`. | No ampliar estos campos ni criterios; reemplazarlos por base común, período objetivo y selector exacto conforme a `INT-FIN-005`. | OWNERS_VIGENTES por alcance: #427–#429/#423 | Configuración pactada vigente | Sí mientras alimente contratos heredados | ALTA |
...
| Documentación comercial | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-COM-007` define registro, modificación, invalidación y consulta de documentación operativa de venta; no existe circuito documental Comercial integral. | Implementar metadata y lifecycle comercial e integrarlos con Documento lógico/versionado sin trasladar ownership del acto Comercial a Documental. | OWNER_ISSUE = FALTA | Subsistema Documental; Venta | Sí completitud legal | ALTA |
| Rescisión | NUCLEO_DOMINIO | PENDIENTE | Tabla mínima, guards y referencias de backlog no constituyen un contrato formal suficiente ni un workflow runtime. | Definir el caso de uso conservador, trazabilidad y efectos financieros/documentales. | OWNER_VIGENTE: #6 | Reversión/pagos | No para venta inicial | MEDIA |
...
| Garantías contractuales locativas | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-LOC-003` contempla registro, modificación, validación, baja lógica, consulta, vigencia, vínculo con el contrato y documentación/evidencia; no existe SQL específico funcional ni circuito application/API/tests. | Implementar soporte físico, application services, API, tests y UI del workflow contractual; cualquier efecto monetario corresponde a Financiero. | OWNER_ISSUE = FALTA | Documental; integración con Financiero sólo para efectos monetarios | Sí completitud contractual | ALTA |
...
| Renovación/extensión | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-LOC-004` define renovación, extensión, continuidad y consulta histórica; el DER materializa la continuidad estructural mediante `contrato_alquiler.id_contrato_anterior`. | Implementar application service, API, runtime, tests, UI y los efectos e integraciones exigidos por el contrato. | OWNER_ISSUE = FALTA | Lifecycle locativo | Sí ciclo contractual completo | ALTA |
| Documentación locativa | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-LOC-006` define registro, modificación, invalidación y consulta de documentación asociada al contrato; no existe circuito runtime funcional. | Implementar metadata locativa, services, API, tests y vínculo con el subsistema Documental sin trasladarle la semántica contractual. | OWNER_ISSUE = FALTA | Documental; garantías/contrato según asociación | Sí completitud contractual | ALTA |
...
| Seguridad locativa | SOPORTE_TRANSVERSAL | IMPLEMENTADO_PARCIALMENTE | Existen guards parciales. | Adoptar Bearer y scope de forma uniforme. | OWNER_VIGENTE de seguridad: #461 | #536 | Sí | CRÍTICA |
| Frontend locativo | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen páginas, ficha y lecturas reales de contratos. | Completar las acciones del ciclo locativo sin inferir avance de Operativo o Administrativo. | OWNER_ISSUE = FALTA UI | Contexto/seguridad | Sí | CRÍTICA |
...
| Consulta de índices/valor aplicable | NUCLEO_DOMINIO | IMPLEMENTADO | Existen readers/repositories, endpoints y tests para consultar índices y resolver el valor aplicable. | Completar el selector exacto por período objetivo sin mezclar gestión ni publicación. | OWNER_VIGENTE de selector: #423; OWNER_VIGENTE de portabilidad: #534 | #427/#429 | Sí PPV2 | CRÍTICA |
| Gestión/publicación de índices y valores | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | Existen tablas físicas, contratos DEV-SRV y datos preparados directamente por tests; eso no constituye un write path productivo. | Implementar commands de alta y modificación, publicación y actualización de valores, validaciones, API, tests productivos y lifecycle portable. | RELACIONADOS_NO_OWNER: #428 consume publicación y #534 requiere portabilidad; OWNER_ISSUE = FALTA para gestión/publicación | Consulta de índices; contratos financieros | Sí para operación administrada y publicación | CRÍTICA |
| PPV2 financiero | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen obligaciones, trazabilidad, indexación y materialización financiera del plan pactado. | Completar base común definitiva, período objetivo, selector exacto y Sync sin absorber el ownership Comercial del plan, sus bloques ni su regla contractual. | OWNERS_VIGENTES por incremento: #427–#429/#423/#534 | Plan/bloques y configuración pactada de Comercial; Comercial portable | Sí | CRÍTICA |
| Cancelación anticipada | NUCLEO_DOMINIO | PENDIENTE | Existen referencias de backlog y modelos de lectura analítica, pero no un contrato operativo financiero suficiente ni runtime. | Definir el caso de uso, liquidación, efectos, trazabilidad y tests integrales. | OWNER_ISSUE = FALTA | Pagos/reversión | No MVP inicial | MEDIA |
| Refinanciación/reprogramación | NUCLEO_DOMINIO | PENDIENTE | Existen categorías y referencias de arquitectura, pero no un contrato operativo suficiente ni runtime de refinanciación/reprogramación. | Definir contrato, modelo físico, lifecycle, trazabilidad, integración y tests. | OWNER_ISSUE = FALTA | Reversión/versionado | No MVP inicial | MEDIA |
| NC/ND | NUCLEO_DOMINIO | PENDIENTE | Existen conceptos de crédito/débito, pero no un contrato formal específico ni circuito runtime de nota de crédito/débito. | Definir emisión, aplicación, comprobante, auditoría y relación con deuda. | OWNER_ISSUE = FALTA | Documental/caja | No MVP inicial | MEDIA |
| Emisión formal/comprobantes | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | Existen contratos y soporte físico/preparatorio; la consulta/proyección de recibo agrupado es read-only y no demuestra emisión materializada. | Implementar command de emisión, persistencia del comprobante, numeración aplicada por runtime, API, tests e integración Documental/fiscal según alcance. | OWNER_ISSUE = FALTA | Documental | Sí formalización | ALTA |
| Tesorería/cuentas/movimientos | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen cuentas financieras y circuitos focales de movimientos de tesorería para egresos de servicios/impuestos, incluida su consulta y anulación; no existe el lifecycle general completo de tesorería. | Completar alta/modificación de cuentas y los restantes ingresos, egresos, ajustes, depósitos, devoluciones, saldos y consultas sin absorber Caja física. | COORDINADOR: #208; OWNER_ISSUE = FALTA focal | Integración Pago ↔ movimiento físico de Caja | Sí control financiero | CRÍTICA |
| Conciliación financiera/bancaria | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-FIN-011` y las tablas preparatorias `conciliacion_bancaria`/`detalle_conciliacion` definen la capacidad; no existe router, application service, repository funcional ni tests del circuito. | Implementar registro, asociación de movimientos, detección de diferencias, consulta, validaciones, API y tests. | COORDINADOR: #208; OWNER_ISSUE = FALTA focal | Tesorería/cuentas/movimientos | Sí control financiero integral | ALTA |
| Garantías monetarias | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | SRV-FIN-011 documenta depósitos/valores monetarios, pero no prueba tabla funcional ni circuito application/API/tests. | Implementar constitución, afectación, liberación, devolución monetaria, ejecución financiera y conciliación relacionada; no confundirlo con el registro contractual locativo ni con Pago ↔ Caja física. | OWNER_ISSUE = FALTA focal | Tesorería; contratos generadores cuando corresponda | Sí para esa cobertura financiera | ALTA |
| Scheduler financiero | SOPORTE_TRANSVERSAL | PENDIENTE | Hay entry points técnicos reutilizables, sin ejecución productiva supervisada. | Automatizar mora y vencimientos financieros que lo requieran. | DEPENDENCIA_IMPORTANTE: #365; OWNER_ISSUE = FALTA para worker | #522 para origen SISTEMA cuando corresponda | Sí automatización | CRÍTICA |
| Sync financiero | SOPORTE_TRANSVERSAL | PENDIENTE | Hay infraestructura Sync común y adopción puntual. | Completar eventos portables y convergencia financiera. | OWNER_VIGENTE por alcance PPV2: #534; OWNER_ISSUE = FALTA general | Identidades portables | Sí multi-base | CRÍTICA |
| Seguridad financiera | SOPORTE_TRANSVERSAL | PENDIENTE | Están disponibles Bearer y scope comunes, con adopción desigual. | Adoptar identidad humana y autorización contextual uniformes. | OWNER_VIGENTE: #461 | #536 | Sí uso humano seguro | CRÍTICA |
| Frontend financiero | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Estado de cuenta, simulación y vistas PPV2 financieras; acción `Pagar`, panel y confirmación de pago, con llamada a `ApiClient.registrar_pago_persona` sobre `/api/v1/financiero/pagos`. | Completar variantes de pago no cubiertas, imputaciones avanzadas, reversión, conciliación integral, mora operativa e índices operativos; la gestión de Caja física permanece en el frontend Operativo. | COORDINADOR: #208; OWNER_VIGENTE de indexación UI: #348; OWNER_ISSUE = FALTA para pagos UI restantes | Backend/seguridad; integración Pago ↔ movimiento físico cuando corresponda | Sí | CRÍTICA |
...
| Caja/apertura/cierre/movimientos | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Caja base, apertura, cierre simple de la apertura, registro y consulta de movimientos. | Implementar cierre consolidado conforme a `cierre_caja`, su persistencia/consulta, anulación de movimientos, arqueo, diferencias, jornada e integración Financiero ↔ Caja cuando corresponda. | COORDINADOR: #248; OWNERS_VIGENTES focales: #256 arqueo/diferencias, #257 jornada y #258 lectura Financiero → Caja; OWNER_ISSUE = FALTA para cierre consolidado/anulación | DEPENDENCIA_IMPORTANTE sólo según flujo: #365 | Sí | CRÍTICA |
| Diferencias de cierre | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `DEV-ARCH-OPE-001` y `SRV-OPE-005` definen totales declarados, comparación, diferencias y cierre consolidado; no existe circuito runtime completo. | Implementar cálculo, persistencia, consulta, validaciones y auditoría de diferencias. | OWNER_VIGENTE: #256; COORDINADOR: #248 | Caja | Sí control | ALTA |
| Arqueo físico | NUCLEO_DOMINIO | PENDIENTE | Figura como alcance pendiente vinculado a #256, pero los contratos vigentes no congelan todavía un circuito completo de arqueo físico. | Definir conteo, responsables, aprobación, tolerancias, relación con cierre y auditoría antes de implementar. | OWNER_VIGENTE: #256; COORDINADOR: #248 | Caja; jornada sólo si el contrato final la exige | Sí control | ALTA |
...
| Lifecycle Sucursal/Instalación | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-OPE-001/002` definen alta, modificación, baja lógica y consulta; el runtime parcial y la metadata existente no materializan el lifecycle completo. | Implementar las operaciones e invariantes ausentes bajo ownership Operativo. | COORDINADOR: #248; #536 excluye lifecycle; OWNER_ISSUE = FALTA focal | Contexto local canónico | Sí ciclo completo | CRÍTICA |
| Sync/multi-instalación de Sucursal/Instalación | SOPORTE_TRANSVERSAL | PENDIENTE | No existe circuito portable autoritativo completo. | Implementar eventos, consumers y resolución remota. | COORDINADOR: #248; OWNER_ISSUE = FALTA focal | Infra Sync; lifecycle Operativo | Sí multi-base | CRÍTICA |
| Consulta operativa consolidada | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-OPE-006` define búsqueda, consulta histórica y reporte consolidado read-only de Sucursales, Instalaciones, cajas, movimientos y cierres; los listados y detalles puntuales existentes no materializan esa vista integrada. | Implementar query/application service consolidado, read model o repository, API/router, tests e integración de fuentes; incorporar el cierre consolidado cuando exista. | OWNER_ISSUE = FALTA | Sucursal, Instalación, Caja, movimientos y cierre consolidado | No para operación puntual; sí para consulta integral | ALTA |
| Compatibilidad legacy de contexto frontend | COMPATIBILIDAD_HEREDADA | LEGACY_VIGENTE | Persisten headers y hardcodes históricos; la infraestructura canónica de PR #537 ya está integrada. | Adoptar sesión, Sucursal elegible e Instalación backend-authoritative mediante #536/#461. | OWNERS_VIGENTES por alcance: #461/#536 | Infraestructura #537 integrada; adopción incremental | Sí | CRÍTICA |
...
| Capacidad | Clasificación | Estado | Qué existe hoy | Qué falta | Owner issue | Dependencias | Bloquea uso real | Prioridad |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DEV-ARCH/DER | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `DEV-ARCH-GOP-001` y el DER de Tarea/Comentario/Historial están documentalmente completos. | Materializar SQL, runtime, API y tests funcionales GOP; los artefactos documentales no cuentan como implementación funcional. | OWNERS_HISTORICOS_COMPLETADOS: #523/#528 | — | No | — |
...
| Almacenamiento físico/binario, hash e integridad | SOPORTE_TRANSVERSAL | PENDIENTE | `archivo_digital` en SQL es soporte preparatorio y no prueba storage operativo; los contratos Documentales vigentes excluyen expresamente el almacenamiento físico/binario de su núcleo. | Formalizar ownership Técnico/Infraestructura e implementar blobs/objetos, almacenamiento, hash, integridad binaria, retención, verificación física, protección y recuperación. | OWNER_ISSUE = FALTA; ownership técnico/infraestructura aún no congelado | Infra/secretos | Sí | CRÍTICA |
| Política documental de acceso/autorización | NUCLEO_DOMINIO | PENDIENTE | Los contratos Documentales exigen permisos de gestión/consulta y control de acceso, pero no existe una política integral formalizada ni runtime uniforme. | Definir e implementar autorización sobre documento lógico y versiones, permisos por asociación/entidad y auditoría de acceso documental. | OWNER_ISSUE = FALTA | #461/#536; asociaciones documentales | Sí | CRÍTICA |
| Transferencia binaria / upload / download / streaming | SOPORTE_TRANSVERSAL | PENDIENTE | No existe contrato técnico formal suficiente ni circuito runtime de transferencia de contenido. | Formalizar ownership Técnico/Infraestructura e implementar upload/download, streaming, límites de tamaño, transporte físico e integridad técnica asociada. | OWNER_ISSUE = FALTA; ownership técnico/infraestructura aún no congelado | Almacenamiento físico/binario; #461/#536 | Sí | CRÍTICA |
| Numeración/emisión | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | Existen soporte físico preparatorio y contratos DEV-SRV, sin circuito funcional utilizable. | Implementar caso de uso, runtime, endpoints, tests e integración documental real. | OWNER_ISSUE = FALTA | Financiero/Comercial | Parcial | ALTA |
| Gestión de plantillas documentales | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-DOC-004` define creación, modificación, invalidación y consulta de `plantilla_documental`; no existe SQL ni circuito runtime funcional. | Implementar soporte físico, application services, API, tests, UI e integración con emisión documental. | OWNER_ISSUE = FALTA | Emisión/numeración documental | Sí para emisión basada en plantillas | ALTA |
| Consulta documental consolidada | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-DOC-006` define búsqueda operativa, consulta histórica, versiones, asociaciones, plantillas, emisiones, numeración, trazabilidad y reporte consolidado read-only; existen tablas preparatorias relacionadas, pero no un circuito funcional consolidado. | Implementar application/query service, repository o read model, API/router, tests, autorización e integración de las fuentes documentales; el consumo UI permanece separado. | OWNER_ISSUE = FALTA | Documento lógico, versiones, asociaciones, plantillas y emisión | Sí para consulta documental integral | ALTA |
...
| Capacidad | Clasificación | Estado | Qué existe hoy | Qué falta | Owner issue | Dependencias | Bloquea uso real | Prioridad |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Arquitectura/servicios | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `DEV-ARCH-ANA-001` y los contratos `SRV-ANA-001..014` definen una capa exclusivamente read-only. | Seleccionar el alcance del MVP, materializar contratos de corte/scope y construir el runtime analítico. | OWNER_ISSUE = FALTA | Fuentes estables | No | MEDIA |
...
| Vistas/consultas/series analíticas | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | DEV-ARCH y DEV-SRV definen consultas/vistas read-only; no hay capa SQL ni runtime analítico propios. | Materializar consultas, vistas ordinarias, cortes y series que consuman fuentes estables exclusivamente en lectura. | OWNER_ISSUE = FALTA | Fuentes estables | No MVP | MEDIA |
...
| Ventas/cobranzas/mora/alquileres/disponibilidad/caja/sucursales | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-ANA-002..014` definen vistas analíticas read-only por área; los datos fuente existen con madurez desigual, pero no hay runtime analítico consolidado. | Materializar modelos de lectura, cortes y pruebas sin duplicar ownership ni recalcular la verdad de los dominios productores. | OWNER_ISSUE = FALTA | Dominios productores | No MVP | MEDIA |
...
| Capacidad | Clasificación | Estado | Qué existe hoy | Qué falta | Owner issue | Dependencias | Bloquea uso real | Prioridad |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Autenticación/sesión frontend | SOPORTE_TRANSVERSAL | PENDIENTE | El backend está disponible para ser consumido, pero el cliente no tiene un circuito materializado de login, logout, manejo de sesión, persistencia segura de token ni envío de `Authorization: Bearer`. | Implementar el circuito completo en frontend sin contabilizar la disponibilidad backend como avance del cliente. | OWNER_VIGENTE de migración humana: #461 | Administrativo | Sí | CRÍTICA |
| Contexto canónico Sucursal/Instalación | SOPORTE_TRANSVERSAL | PENDIENTE | El backend aportado por PR #537 está disponible como prerequisito, pero el frontend conserva IDs fijos y no tiene selector de Sucursal ni sesión/contexto backend-authoritative materializados. | Implementar selector de Sucursal elegible/autorizada, sesión y contexto operacional; usar Instalación resuelta por backend y `X-Instalacion-Id` sólo como aserción transicional. | OWNERS_VIGENTES por alcance: #536/#461 | Infraestructura backend integrada; migraciones incrementales | Sí | CRÍTICA |
...
| Frontend Personas | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen listas, fichas y gestión visible de datos relacionados; `ApiClient.crear_persona` por sí solo no constituye una superficie de alta. | Incorporar alta de Persona en la navegación/UI y completar lifecycle, relaciones, poderes y acciones. | OWNERS_VIGENTES frontend: #206/#244 | Seguridad | Sí | ALTA |
| Frontend Inmobiliario | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen listas, fichas, altas e importación relevantes. | Completar lifecycle y acciones inmobiliarias. | OWNER_VIGENTE frontend: #198 | Seguridad | Sí | ALTA |
| Frontend Comercial | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen wizard, ventas y superficies PPV2. | Completar roadmap #430/#431 y actos posteriores. | OWNERS_VIGENTES/COORDINADORES: #207/#430/#431 | Backend pendiente | Sí | CRÍTICA |
| Frontend Financiero | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen estado de cuenta, simulación y registro básico de pagos con panel de confirmación y llamada al endpoint financiero. | Completar variantes no cubiertas, imputaciones avanzadas, reversión, conciliación, mora e índices operativos. | COORDINADOR: #208 | Backend/seguridad | Sí | CRÍTICA |
| Frontend Locativo | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Existen páginas y lecturas reales de contratos. | Completar mutaciones locativas. | OWNER_ISSUE = FALTA UI focal | Contexto/seguridad | Sí | CRÍTICA |
| Frontend Operativo | NUCLEO_DOMINIO | PENDIENTE | No existe gestión UI real de Caja, Jornada, Sucursal, Instalación o configuración operativa. | Implementar las superficies bajo ownership Operativo. | COORDINADOR: #248; OWNER_ISSUE = FALTA UI focal | Contexto/seguridad | Sí | CRÍTICA |
| Frontend Administrativo | NUCLEO_DOMINIO | PENDIENTE | No existe UI integral de usuarios, roles, permisos, parámetros, calendario y auditoría. | Implementar las superficies bajo ownership Administrativo, sin absorber Sucursal/Instalación. | COORDINADOR: #249; OWNER_ISSUE = FALTA UI focal | Seguridad/contexto | Sí | CRÍTICA |
| Frontend GOP | NUCLEO_DOMINIO | PENDIENTE | No existe módulo funcional. | Implementar después de sus contratos y backend MVP. | COORDINADOR sólo post-MVP: #527; OWNER_ISSUE = FALTA para MVP | GOP backend | No MVP core | MEDIA |
| Frontend Documental | NUCLEO_DOMINIO | PENDIENTE | No existe módulo funcional. | Implementar después del subsistema documental mínimo. | OWNER_ISSUE = FALTA | Backend Documental | Sí para documental mínimo usable | MEDIA |
| Frontend Analítico | NUCLEO_DOMINIO | PENDIENTE | No existe módulo funcional. | Implementar después de definir MVP y backend Analítico. | OWNER_ISSUE = FALTA | Backend Analítico | No MVP core | MEDIA |

## 15. Operación, deploy e infraestructura productiva

| Capacidad | Clasificación | Estado | Qué existe hoy | Qué falta | Owner issue | Dependencias | Bloquea uso real | Prioridad |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Provisionamiento técnico de deployment/base | SOPORTE_TRANSVERSAL | IMPLEMENTADO_PARCIALMENTE | `.env.example`, settings y scripts de creación/reset de DB DEV/TEST. | Completar bootstrap técnico idempotente por instalación/deployment y gestión segura de configuración y secretos, sin incluir Usuario ni Credencial. | OWNER_ISSUE = FALTA | #536/secretos | Sí | CRÍTICA |
| Workers/scheduler | SOPORTE_TRANSVERSAL | PENDIENTE | Entry points y lógica invocable. | Servicio supervisado para inbox, retry, mora y vencimientos que requieran automatización. | OWNER_ISSUE = FALTA | #522/#365 | Sí | CRÍTICA |
...
| Backup/restore | SOPORTE_TRANSVERSAL | DOCUMENTADO_NO_IMPLEMENTADO | `SRV-TEC-006` contrata respaldo, recuperación, verificación de integridad, persistencia de operaciones y consultas de estado; no existe circuito SQL/application/API/tests funcional. | Implementar ejecución de backup y restore; cifrado/protección, almacenamiento y retención; verificación de integridad y compatibilidad; comprobación de consistencia posterior a la recuperación; registro persistente de respaldos, restores y verificaciones; consulta de estados/resultados; tests y runbook productivo. | OWNER_ISSUE = FALTA | Storage/secretos | Sí producción | CRÍTICA |
...
| Gestión operable de conflictos Sync | Técnico/Sync | ALTA | FALTA | Sí, separado de los conflictos terminales puntuales de inbox. |
| Gestión de jobs e intercambio técnico por lotes | Técnico/Sync | ALTA | FALTA | Sí, separado del runner/scheduler productivo y del importador Excel funcional. |
| Autorizaciones administrativas explícitas | Administrativo | MEDIA | FALTA focal | Sí, sin duplicar autorización de acceso GLOBAL/por Sucursal. |
...
| Relaciones estructurales y documentación inmobiliaria pendiente | Inmobiliario | MEDIA | FALTA | Sí, sólo cuando se cierre su contrato operativo. |
| Documentación comercial integrada | Comercial/Documental | ALTA | FALTA | Sí, preservando la semántica Comercial. |
| Garantías locativas end-to-end | Locativo/Financiero | ALTA | FALTA | Sí. |
| Cierre de renovación/extensión y rescisión locativa | Locativo | ALTA | FALTA | Sí, posiblemente dividido. |
| Documentación contractual locativa | Locativo/Documental | ALTA | FALTA | Sí, preservando ownership locativo de la semántica. |
| Conciliación financiera/bancaria | Financiero | ALTA | FALTA focal | Sí. |
...
| Gestión de plantillas documentales | Documental | ALTA | FALTA | Sí, coordinado con emisión/numeración. |
