...
| Garantías contractuales locativas | NUCLEO_DOMINIO | DOCUMENTADO_NO_IMPLEMENTADO | DEV-SRV contempla registro, tipo, validación, vigencia, vínculo con el contrato y documentación/evidencia; no existe SQL específico funcional ni circuito application/API/tests. | Diseñar el soporte físico y workflow contractual; cualquier efecto monetario corresponde a Financiero. | OWNER_ISSUE = FALTA | Documental; integración con Financiero sólo para efectos monetarios | Sí completitud contractual | ALTA |
...
| Tesorería/cuentas/movimientos/conciliación | NUCLEO_DOMINIO | IMPLEMENTADO_PARCIALMENTE | Financiero posee estructuras y servicios parciales para cuentas, movimientos de tesorería y conciliación financiera. | Completar el lifecycle y la conciliación financiera sin absorber Caja física. | COORDINADOR: #208; OWNER_ISSUE = FALTA focal | Integración Pago ↔ movimiento físico de Caja | Sí control financiero | CRÍTICA |
...
| Lifecycle Sucursal/Instalación | NUCLEO_DOMINIO | PENDIENTE | `uid_global` y metadata no constituyen un lifecycle completo. | Completar operaciones e invariantes gobernadas por Operativo. | COORDINADOR: #248; #536 excluye lifecycle; OWNER_ISSUE = FALTA focal | Contexto local canónico | Sí ciclo completo | CRÍTICA |
