...
#448 prepara exclusivamente `public.credencial_usuario` mediante ALTER directo incremental: metadata CORE-EF física, constraints, FKs de instalación, índices parciales y triggers SQL. No crea credenciales reales, no modifica `usuario`, no agrega Argon2id, login, logout, sesiones, tokens, outbox ni historial runtime. `hash_credencial` es sensible y usuarios sin credencial no autentican porque no existe runtime de autenticación. #449 y #450 siguen pendientes; #446 permanece bloqueado.
