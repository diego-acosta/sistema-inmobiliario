...
--
-- Name: comprobante_impuesto; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.comprobante_impuesto (
    ...
    observaciones text,
    CONSTRAINT chk_comprobante_impuesto_estado CHECK (((estado_comprobante_impuesto)::text = ANY ((ARRAY['REGISTRADO'::character varying, 'ANULADO'::character varying])::text[]))),
    CONSTRAINT chk_comprobante_impuesto_fechas CHECK (((fecha_emision IS NULL) OR (fecha_vencimiento >= fecha_emision))),
    CONSTRAINT chk_comprobante_impuesto_importe CHECK ((importe_total >= (0)::numeric)),
    CONSTRAINT chk_comprobante_impuesto_modalidad CHECK (((modalidad_gestion_impuesto)::text = ANY ((ARRAY['EMPRESA_ASUME'::character varying, 'DIRECTO_RESPONSABLE'::character varying, 'EMPRESA_PAGA_Y_RECUPERA'::character varying])::text[]))),
    CONSTRAINT chk_comprobante_impuesto_objeto_xor CHECK ((((id_inmueble IS NOT NULL) AND (id_unidad_funcional IS NULL)) OR ((id_inmueble IS NULL) AND (id_unidad_funcional IS NOT NULL)))),
    CONSTRAINT chk_comprobante_impuesto_periodo CHECK (((periodo_hasta IS NULL) OR (periodo_desde IS NULL) OR (periodo_hasta >= periodo_desde))),
    CONSTRAINT uq_comprobante_impuesto_uid_global UNIQUE (uid_global)
);
...
CREATE TABLE public.egreso_proveedor_factura_servicio (
...
CREATE TABLE public.egreso_impuesto_empresa (
    ...
    estado_egreso character varying(30) NOT NULL DEFAULT 'REGISTRADO',
    observaciones text,
    CONSTRAINT chk_eie_importe CHECK (importe_pagado > 0),
    CONSTRAINT chk_eie_estado CHECK (estado_egreso IN ('REGISTRADO', 'ANULADO'))
);
...
--
-- Name: liquidacion_recupero; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.liquidacion_recupero (
    ...
    CONSTRAINT chk_lr_importes CHECK ((importe_total_egresado_base > 0) AND (importe_total_recuperar > 0) AND (importe_absorbido_empresa >= 0) AND (importe_total_recuperar <= importe_total_egresado_base))
);


--
-- Name: liquidacion_recupero_factura; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.liquidacion_recupero_factura (
    ...
    CONSTRAINT chk_lrf_importes CHECK ((importe_egresado_base > 0) AND (importe_recuperar > 0) AND (importe_recuperar <= importe_egresado_base))
);


--
-- Name: liquidacion_recupero_egreso; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.liquidacion_recupero_egreso (
    ...
    estado_liquidacion_recupero_egreso character varying(30) NOT NULL DEFAULT 'ACTIVO',
    CONSTRAINT chk_lre_estado CHECK (estado_liquidacion_recupero_egreso IN ('ACTIVO', 'ANULADO')),
    CONSTRAINT chk_lre_importe CHECK (importe_imputado_base > 0)
);


--
-- Name: liquidacion_recupero_responsable; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.liquidacion_recupero_responsable (
...
CREATE TABLE public.liquidacion_impuesto_trasladado (
    ...
    id_obligacion_financiera bigint,
    observaciones text,
    CONSTRAINT chk_lit_estado CHECK (estado_liquidacion IN ('EMITIDA', 'ANULADA')),
    CONSTRAINT chk_lit_modalidad CHECK (modalidad_gestion_impuesto IN ('DIRECTO_RESPONSABLE', 'EMPRESA_PAGA_Y_RECUPERA')),
    CONSTRAINT chk_lit_importes CHECK (importe_total_base >= 0 AND importe_total_trasladar > 0 AND importe_absorbido_empresa >= 0),
    CONSTRAINT chk_lit_fechas CHECK (fecha_vencimiento >= fecha_liquidacion)
);


CREATE TABLE public.liquidacion_impuesto_trasladado_comprobante (
    ...
    CONSTRAINT chk_litc_importes CHECK (importe_comprobante >= 0 AND importe_base >= 0 AND importe_trasladar > 0)
);


CREATE TABLE public.liquidacion_impuesto_trasladado_egreso (
    ...
    CONSTRAINT chk_lite_importe CHECK (importe_imputado_base > 0),
    CONSTRAINT chk_lite_estado CHECK (estado_liquidacion_impuesto_egreso IN ('ACTIVO', 'ANULADO'))
);


CREATE TABLE public.liquidacion_impuesto_trasladado_responsable (
    ...
    importe_responsable numeric(14,2) NOT NULL,
    origen_responsable character varying(50) NOT NULL DEFAULT 'MANUAL',
    CONSTRAINT chk_litr_porcentaje CHECK (porcentaje_responsabilidad > 0 AND porcentaje_responsabilidad <= 100),
    CONSTRAINT chk_litr_importe CHECK (importe_responsable >= 0)
);
...
ALTER TABLE ONLY public.venta
    ADD CONSTRAINT chk_venta_tipo_plan_financiero CHECK (((tipo_plan_financiero)::text = ANY ((ARRAY['CONTADO'::character varying, 'ANTICIPO_Y_SALDO'::character varying, 'CUOTAS_FIJAS'::character varying])::text[])));
...
--
-- Name: idx_epfs_factura_servicio; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_epfs_factura_servicio ON public.egreso_proveedor_factura_servicio USING btree (id_factura_servicio);


--
-- Name: idx_epfs_movimiento_tesoreria; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_epfs_movimiento_tesoreria ON public.egreso_proveedor_factura_servicio USING btree (id_movimiento_tesoreria);


--
-- Name: idx_epfs_uid_global; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_epfs_uid_global ON public.egreso_proveedor_factura_servicio USING btree (uid_global);


--
-- Name: ux_epfs_op_id_alta_activo; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_epfs_op_id_alta_activo ON public.egreso_proveedor_factura_servicio USING btree (op_id_alta) WHERE ((op_id_alta IS NOT NULL) AND (deleted_at IS NULL));


--
-- Name: idx_eie_comprobante_impuesto; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_eie_comprobante_impuesto ON public.egreso_impuesto_empresa USING btree (id_comprobante_impuesto);


--
-- Name: idx_eie_movimiento_tesoreria; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_eie_movimiento_tesoreria ON public.egreso_impuesto_empresa USING btree (id_movimiento_tesoreria);


--
-- Name: idx_eie_uid_global; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_eie_uid_global ON public.egreso_impuesto_empresa USING btree (uid_global);


--
-- Name: ux_eie_op_id_alta_activo; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_eie_op_id_alta_activo ON public.egreso_impuesto_empresa USING btree (op_id_alta) WHERE ((op_id_alta IS NOT NULL) AND (deleted_at IS NULL));


--
-- Name: idx_lr_relacion_generadora; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lr_relacion_generadora ON public.liquidacion_recupero USING btree (id_relacion_generadora);


--
-- Name: idx_lr_obligacion_financiera; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lr_obligacion_financiera ON public.liquidacion_recupero USING btree (id_obligacion_financiera);


--
-- Name: idx_lrf_factura_servicio; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lrf_factura_servicio ON public.liquidacion_recupero_factura USING btree (id_factura_servicio);


--
-- Name: ux_lre_egreso_activo; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_lre_egreso_activo ON public.liquidacion_recupero_egreso USING btree (id_egreso_proveedor_factura_servicio) WHERE ((deleted_at IS NULL) AND ((estado_liquidacion_recupero_egreso)::text = 'ACTIVO'::text));


--
-- Name: idx_lrr_persona; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_lrr_persona ON public.liquidacion_recupero_responsable USING btree (id_persona);


CREATE INDEX idx_lit_relacion_generadora ON public.liquidacion_impuesto_trasladado USING btree (id_relacion_generadora);


CREATE INDEX idx_lit_obligacion_financiera ON public.liquidacion_impuesto_trasladado USING btree (id_obligacion_financiera);


CREATE INDEX idx_litc_comprobante ON public.liquidacion_impuesto_trasladado_comprobante USING btree (id_comprobante_impuesto);


CREATE INDEX idx_lite_egreso ON public.liquidacion_impuesto_trasladado_egreso USING btree (id_egreso_impuesto_empresa);


CREATE INDEX idx_litr_persona ON public.liquidacion_impuesto_trasladado_responsable USING btree (id_persona);


CREATE UNIQUE INDEX ux_lite_egreso_activo ON public.liquidacion_impuesto_trasladado_egreso USING btree (id_egreso_impuesto_empresa) WHERE ((deleted_at IS NULL) AND ((estado_liquidacion_impuesto_egreso)::text = 'ACTIVO'::text));


CREATE UNIQUE INDEX ux_lit_codigo_activo ON public.liquidacion_impuesto_trasladado USING btree (codigo_liquidacion_impuesto_trasladado) WHERE ((codigo_liquidacion_impuesto_trasladado IS NOT NULL) AND (deleted_at IS NULL));


CREATE UNIQUE INDEX ux_lit_op_id_alta_activo ON public.liquidacion_impuesto_trasladado USING btree (op_id_alta) WHERE ((op_id_alta IS NOT NULL) AND (deleted_at IS NULL));


--
-- Name: ux_lr_codigo_activo; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_lr_codigo_activo ON public.liquidacion_recupero USING btree (codigo_liquidacion_recupero) WHERE ((codigo_liquidacion_recupero IS NOT NULL) AND (deleted_at IS NULL));


--
-- Name: ux_lr_op_id_alta_activo; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_lr_op_id_alta_activo ON public.liquidacion_recupero USING btree (op_id_alta) WHERE ((op_id_alta IS NOT NULL) AND (deleted_at IS NULL));
...
--
-- Name: idx_comprobante_impuesto_deleted_at; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_deleted_at ON public.comprobante_impuesto USING btree (deleted_at);


--
-- Name: idx_comprobante_impuesto_fecha_vencimiento; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_fecha_vencimiento ON public.comprobante_impuesto USING btree (fecha_vencimiento);


--
-- Name: idx_comprobante_impuesto_id_inmueble; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_id_inmueble ON public.comprobante_impuesto USING btree (id_inmueble);


--
-- Name: idx_comprobante_impuesto_id_unidad_funcional; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_id_unidad_funcional ON public.comprobante_impuesto USING btree (id_unidad_funcional);


--
-- Name: idx_comprobante_impuesto_modalidad; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_modalidad ON public.comprobante_impuesto USING btree (modalidad_gestion_impuesto);


--
-- Name: idx_comprobante_impuesto_organismo; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_organismo ON public.comprobante_impuesto USING btree (organismo);


--
-- Name: idx_comprobante_impuesto_uid_global; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_comprobante_impuesto_uid_global ON public.comprobante_impuesto USING btree (uid_global);
...
--
-- Name: ux_instalacion_op_id_alta; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_instalacion_op_id_alta ON public.instalacion USING btree (op_id_alta) WHERE (op_id_alta IS NOT NULL);
...
--
-- Name: ux_comprobante_impuesto_activo_organismo_numero; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX ux_comprobante_impuesto_activo_organismo_numero ON public.comprobante_impuesto USING btree (organismo, numero_comprobante) WHERE (deleted_at IS NULL);
...
--
-- Name: comprobante_impuesto trg_bi_comprobante_impuesto_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bi_comprobante_impuesto_core_ef BEFORE INSERT ON public.comprobante_impuesto FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_insert();
...
--
-- Name: egreso_proveedor_factura_servicio trg_bi_epfs_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bi_epfs_core_ef BEFORE INSERT ON public.egreso_proveedor_factura_servicio FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_insert();


--
-- Name: egreso_impuesto_empresa trg_bi_eie_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bi_eie_core_ef BEFORE INSERT ON public.egreso_impuesto_empresa FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_insert();


--
-- Name: liquidacion_recupero trg_bi_lr_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bi_lr_core_ef BEFORE INSERT ON public.liquidacion_recupero FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_insert();


--
-- Name: liquidacion_recupero_egreso trg_bi_lre_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bi_lre_core_ef BEFORE INSERT ON public.liquidacion_recupero_egreso FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_insert();
...
--
-- Name: comprobante_impuesto trg_bu_comprobante_impuesto_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bu_comprobante_impuesto_core_ef BEFORE UPDATE ON public.comprobante_impuesto FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_update();
...
--
-- Name: egreso_proveedor_factura_servicio trg_bu_epfs_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bu_epfs_core_ef BEFORE UPDATE ON public.egreso_proveedor_factura_servicio FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_update();


--
-- Name: egreso_impuesto_empresa trg_bu_eie_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bu_eie_core_ef BEFORE UPDATE ON public.egreso_impuesto_empresa FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_update();


--
-- Name: liquidacion_recupero trg_bu_lr_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bu_lr_core_ef BEFORE UPDATE ON public.liquidacion_recupero FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_update();


--
-- Name: liquidacion_recupero_egreso trg_bu_lre_core_ef; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER trg_bu_lre_core_ef BEFORE UPDATE ON public.liquidacion_recupero_egreso FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_update();
...
--
-- Name: egreso_proveedor_factura_servicio fk_epfs_factura_servicio; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.egreso_proveedor_factura_servicio
    ADD CONSTRAINT fk_epfs_factura_servicio FOREIGN KEY (id_factura_servicio) REFERENCES public.factura_servicio(id_factura_servicio) ON DELETE RESTRICT;


--
-- Name: egreso_proveedor_factura_servicio fk_epfs_movimiento_tesoreria; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.egreso_proveedor_factura_servicio
    ADD CONSTRAINT fk_epfs_movimiento_tesoreria FOREIGN KEY (id_movimiento_tesoreria) REFERENCES public.movimiento_tesoreria(id_movimiento_tesoreria) ON DELETE RESTRICT;


--
-- Name: egreso_impuesto_empresa fk_eie_comprobante_impuesto; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.egreso_impuesto_empresa
    ADD CONSTRAINT fk_eie_comprobante_impuesto FOREIGN KEY (id_comprobante_impuesto) REFERENCES public.comprobante_impuesto(id_comprobante_impuesto) ON DELETE RESTRICT;


--
-- Name: egreso_impuesto_empresa fk_eie_movimiento_tesoreria; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.egreso_impuesto_empresa
    ADD CONSTRAINT fk_eie_movimiento_tesoreria FOREIGN KEY (id_movimiento_tesoreria) REFERENCES public.movimiento_tesoreria(id_movimiento_tesoreria) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero fk_lr_obligacion_financiera; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero
    ADD CONSTRAINT fk_lr_obligacion_financiera FOREIGN KEY (id_obligacion_financiera) REFERENCES public.obligacion_financiera(id_obligacion_financiera) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero fk_lr_relacion_generadora; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero
    ADD CONSTRAINT fk_lr_relacion_generadora FOREIGN KEY (id_relacion_generadora) REFERENCES public.relacion_generadora(id_relacion_generadora) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero_factura fk_lrf_factura_servicio; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_factura
    ADD CONSTRAINT fk_lrf_factura_servicio FOREIGN KEY (id_factura_servicio) REFERENCES public.factura_servicio(id_factura_servicio) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero_factura fk_lrf_liquidacion; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_factura
    ADD CONSTRAINT fk_lrf_liquidacion FOREIGN KEY (id_liquidacion_recupero) REFERENCES public.liquidacion_recupero(id_liquidacion_recupero) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero_egreso fk_lre_egreso; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_egreso
    ADD CONSTRAINT fk_lre_egreso FOREIGN KEY (id_egreso_proveedor_factura_servicio) REFERENCES public.egreso_proveedor_factura_servicio(id_egreso_proveedor_factura_servicio) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero_egreso fk_lre_liquidacion; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_egreso
    ADD CONSTRAINT fk_lre_liquidacion FOREIGN KEY (id_liquidacion_recupero) REFERENCES public.liquidacion_recupero(id_liquidacion_recupero) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero_responsable fk_lrr_liquidacion; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_responsable
    ADD CONSTRAINT fk_lrr_liquidacion FOREIGN KEY (id_liquidacion_recupero) REFERENCES public.liquidacion_recupero(id_liquidacion_recupero) ON DELETE RESTRICT;


--
-- Name: liquidacion_recupero_responsable fk_lrr_persona; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_responsable
    ADD CONSTRAINT fk_lrr_persona FOREIGN KEY (id_persona) REFERENCES public.persona(id_persona) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado
    ADD CONSTRAINT fk_lit_relacion_generadora FOREIGN KEY (id_relacion_generadora) REFERENCES public.relacion_generadora(id_relacion_generadora) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado
    ADD CONSTRAINT fk_lit_obligacion_financiera FOREIGN KEY (id_obligacion_financiera) REFERENCES public.obligacion_financiera(id_obligacion_financiera) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado_comprobante
    ADD CONSTRAINT fk_litc_liquidacion FOREIGN KEY (id_liquidacion_impuesto_trasladado) REFERENCES public.liquidacion_impuesto_trasladado(id_liquidacion_impuesto_trasladado) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado_comprobante
    ADD CONSTRAINT fk_litc_comprobante FOREIGN KEY (id_comprobante_impuesto) REFERENCES public.comprobante_impuesto(id_comprobante_impuesto) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado_egreso
    ADD CONSTRAINT fk_lite_liquidacion FOREIGN KEY (id_liquidacion_impuesto_trasladado) REFERENCES public.liquidacion_impuesto_trasladado(id_liquidacion_impuesto_trasladado) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado_egreso
    ADD CONSTRAINT fk_lite_egreso FOREIGN KEY (id_egreso_impuesto_empresa) REFERENCES public.egreso_impuesto_empresa(id_egreso_impuesto_empresa) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado_responsable
    ADD CONSTRAINT fk_litr_liquidacion FOREIGN KEY (id_liquidacion_impuesto_trasladado) REFERENCES public.liquidacion_impuesto_trasladado(id_liquidacion_impuesto_trasladado) ON DELETE RESTRICT;


ALTER TABLE ONLY public.liquidacion_impuesto_trasladado_responsable
    ADD CONSTRAINT fk_litr_persona FOREIGN KEY (id_persona) REFERENCES public.persona(id_persona) ON DELETE RESTRICT;
...
--
-- Name: comprobante_impuesto fk_comprobante_impuesto_inmueble; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.comprobante_impuesto
    ADD CONSTRAINT fk_comprobante_impuesto_inmueble FOREIGN KEY (id_inmueble) REFERENCES public.inmueble(id_inmueble) ON DELETE RESTRICT;


--
-- Name: comprobante_impuesto fk_comprobante_impuesto_unidad_funcional; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.comprobante_impuesto
    ADD CONSTRAINT fk_comprobante_impuesto_unidad_funcional FOREIGN KEY (id_unidad_funcional) REFERENCES public.unidad_funcional(id_unidad_funcional) ON DELETE RESTRICT;
...
--
-- Patch inline: asignacion_servicio_responsable V1
--
...
CREATE TABLE IF NOT EXISTS public.asignacion_servicio_responsable (
    ...
    observaciones text,
    CONSTRAINT chk_asr_objeto_xor CHECK ((((id_inmueble IS NOT NULL) AND (id_unidad_funcional IS NULL)) OR ((id_inmueble IS NULL) AND (id_unidad_funcional IS NOT NULL)))),
    CONSTRAINT chk_asr_porcentaje CHECK (((porcentaje_responsabilidad > (0)::numeric) AND (porcentaje_responsabilidad <= (100)::numeric))),
    CONSTRAINT chk_asr_vigencia CHECK (((fecha_hasta IS NULL) OR (fecha_hasta >= fecha_desde))),
    CONSTRAINT chk_asr_estado CHECK (((estado_asignacion)::text = ANY ((ARRAY['ACTIVA'::character varying, 'INACTIVA'::character varying])::text[]))),
    CONSTRAINT fk_asr_servicio FOREIGN KEY (id_servicio) REFERENCES public.servicio(id_servicio) ON DELETE RESTRICT,
    CONSTRAINT fk_asr_inmueble FOREIGN KEY (id_inmueble) REFERENCES public.inmueble(id_inmueble) ON DELETE RESTRICT,
    CONSTRAINT fk_asr_unidad_funcional FOREIGN KEY (id_unidad_funcional) REFERENCES public.unidad_funcional(id_unidad_funcional) ON DELETE RESTRICT,
    CONSTRAINT fk_asr_persona FOREIGN KEY (id_persona) REFERENCES public.persona(id_persona) ON DELETE RESTRICT,
    CONSTRAINT uq_asignacion_servicio_responsable_uid_global UNIQUE (uid_global)
);

CREATE INDEX IF NOT EXISTS idx_asr_servicio_inmueble_vigencia ON public.asignacion_servicio_responsable USING btree (id_servicio, id_inmueble, fecha_desde, fecha_hasta);
CREATE INDEX IF NOT EXISTS idx_asr_servicio_unidad_vigencia ON public.asignacion_servicio_responsable USING btree (id_servicio, id_unidad_funcional, fecha_desde, fecha_hasta);
CREATE INDEX IF NOT EXISTS idx_asr_persona ON public.asignacion_servicio_responsable USING btree (id_persona);
CREATE INDEX IF NOT EXISTS idx_asr_uid_global ON public.asignacion_servicio_responsable USING btree (uid_global);

DROP TRIGGER IF EXISTS trg_bi_asignacion_servicio_responsable_core_ef ON public.asignacion_servicio_responsable;
CREATE TRIGGER trg_bi_asignacion_servicio_responsable_core_ef BEFORE INSERT ON public.asignacion_servicio_responsable FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_insert();

DROP TRIGGER IF EXISTS trg_bu_asignacion_servicio_responsable_core_ef ON public.asignacion_servicio_responsable;
CREATE TRIGGER trg_bu_asignacion_servicio_responsable_core_ef BEFORE UPDATE ON public.asignacion_servicio_responsable FOR EACH ROW EXECUTE FUNCTION public.trg_core_ef_sync_defaults_update();

--
-- Name: liquidacion_recupero_responsable fk_lrr_asignacion; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.liquidacion_recupero_responsable
    ADD CONSTRAINT fk_lrr_asignacion FOREIGN KEY (id_asignacion_servicio_responsable) REFERENCES public.asignacion_servicio_responsable(id_asignacion_servicio_responsable) ON DELETE RESTRICT;
