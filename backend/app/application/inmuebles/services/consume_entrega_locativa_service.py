...
PROCESSOR_NAME = "inmobiliario.entrega_locativa"
...
class InmuebleRepository(Protocol):
    ...
    def create_ocupacion_sin_commit(self, payload: OcupacionPayload) -> dict[str, Any]: ...
...
class ConsumeEntregaLocativaService:
    def __init__(
        self,
        db: Session,
    ...
    def _consume_single_event(self, event: dict[str, Any]) -> AppResult[dict[str, Any]]:
        ...
        try:
            ...
            fecha_entrega_raw = payload.get("fecha_entrega")
            if not isinstance(fecha_entrega_raw, str):
                return self._reject_business(event, event_uuid, "INVALID_EVENT_DATE")
            try:
                fecha_entrega_date = date.fromisoformat(fecha_entrega_raw)
                fecha_entrega_dt = datetime(
                    fecha_entrega_date.year,
                    fecha_entrega_date.month,
                    fecha_entrega_date.day,
                )
            except ValueError:
                return self._reject_business(event, event_uuid, "INVALID_EVENT_DATE")
            ...
            if not claimed:
                # Ya fue reclamado por este consumer — marcar outbox sin reprocessar
            ...
            # ── procesar objetos: ocupacion + disponibilidad ───────────────────
            object_results: list[dict[str, Any]] = []
            for obj in objetos:
                id_inm = obj.get("id_inmueble")
                id_uf = obj.get("id_unidad_funcional")

                self.inmueble_repository.create_ocupacion_sin_commit(
                    OcupacionPayload(
                        id_inmueble=id_inm,
                        id_unidad_funcional=id_uf,
                        tipo_ocupacion=TIPO_OCUPACION,
                        fecha_desde=fecha_entrega_dt,
                        fecha_hasta=None,
                        descripcion="Ocupación por entrega de contrato de alquiler",
                        observaciones=None,
                        uid_global=str(self.uuid_generator()),
                        version_registro=1,
                        created_at=now,
