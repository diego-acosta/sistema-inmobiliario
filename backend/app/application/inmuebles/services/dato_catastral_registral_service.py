...
def _validate_values(values: dict[str, Any]) -> list[str]:
    ...
    if (
        values.get("fecha_desde")
        and values.get("fecha_hasta")
        and values["fecha_hasta"] < values["fecha_desde"]
    ):
...
def _context_values(command: Any) -> tuple[int | None, UUID | None]:
    return (
        getattr(command.context, "id_instalacion", None),
        getattr(command.context, "op_id", None),
    )
...
class DatoCatastralRegistralService:
    ...
    def list(self, id_inmueble: int) -> AppResult[list[dict[str, Any]]]:
        ...
        return AppResult.ok(
            self.repository.list_datos_catastrales_registrales(id_inmueble)
        )

    def create(
        self, command: CreateDatoCatastralRegistralCommand
        ...
        payload = DatoCatastralRegistralPayload(
            id_inmueble=command.id_inmueble,
            values=values,
            version_registro_actual=None,
            version_registro_nueva=1,
    ...
    def update(
        ...
        current = self.repository.get_dato_catastral_registral(
            command.id_inmueble,
            command.id_dato_catastral_registral,
        )
        if current is None:
            if not self.repository.inmueble_exists(command.id_inmueble):
                return AppResult.fail("NOT_FOUND_INMUEBLE")
        ...
        payload = DatoCatastralRegistralPayload(
            ...
            id_instalacion=id_instalacion,
            op_id=op_id,
            id_dato_catastral_registral=command.id_dato_catastral_registral,
        )
    ...
    def baja(
        self, command: BajaDatoCatastralRegistralCommand
    ) -> AppResult[dict[str, Any]]:
        current = self.repository.get_dato_catastral_registral(
            command.id_inmueble,
            command.id_dato_catastral_registral,
