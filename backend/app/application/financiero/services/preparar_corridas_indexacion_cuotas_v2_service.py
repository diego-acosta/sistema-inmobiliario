...
class PrepararCorridasIndexacionCuotasV2Service:
    def __init__(
        self,
        repository: PrepararCorridasIndexacionCuotasV2Repository,
        preview_service: PreviewIndexacionCuotasV2Service,
    ) -> None:
    ...
    def execute(
        self,
        command: PrepararCorridasIndexacionCuotasV2Command,
        core_ef: CoreEFHeaders,
        ...
        fecha_corte = command.fecha_corte or periodo_aplicado
        configs = self.repository.list_configuraciones_alcanzadas(
            valor["id_indice_financiero"], periodo_aplicado, fecha_corte
        )
        ...
        for cfg in configs:
            ...
            if existente is not None:
                ...
                continue
            op_id_grupo = _derive_group_op_id(core_ef.x_op_id, base)
            group_core = CoreEFHeaders(
                x_op_id=op_id_grupo,
                x_usuario_id=core_ef.x_usuario_id,
                x_sucursal_id=core_ef.x_sucursal_id,
                x_instalacion_id=core_ef.x_instalacion_id,
                if_match_version=None,
            )
            result = self.preview_service.execute(
                PreviewIndexacionCuotasV2Command(
                    **base,
                    fecha_corte=fecha_corte,
                    persistir=True,
                    origen_corrida=ORIGEN_PUBLICACION_INDICE,
                    motivo=command.motivo or "Preparación automática por publicación de índice financiero",
                ),
                group_core,
            )
            if result.success:
                ...
                resultados.append({
                    **base,
                    "id_corrida_indexacion_financiera": data["id_corrida_indexacion_financiera"],
                    "hash_corrida": data["hash_corrida"],
                    "estado_corrida": "PREVISUALIZADA",
                    "cantidad_analizada": data["resumen"]["cantidad_analizada"],
                    "cantidad_elegible": data["resumen"]["cantidad_elegible"],
                    "resultado": "CREADA",
                    "error": None,
                })
                continue
        ...
        return AppResult.ok({
            "id_indice_financiero_valor": command.id_indice_financiero_valor,
            "id_indice_financiero": valor["id_indice_financiero"],
            "periodo_aplicado": periodo_aplicado,
            "fecha_corte": fecha_corte,
            "cantidad_configuraciones_analizadas": len(configs),
            "cantidad_corridas_creadas": creadas,
            "cantidad_corridas_existentes": existentes,
            "cantidad_sin_obligaciones": sin_obligaciones,
...
def _row_resumen(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "id_corrida_indexacion_financiera": row["id_corrida_indexacion_financiera"],
        "hash_corrida": row["hash_corrida"],
        "estado_corrida": row["estado_corrida"],
        "cantidad_analizada": row.get("cantidad_analizada", 0),
        "cantidad_elegible": row.get("cantidad_elegible", 0),
    }
