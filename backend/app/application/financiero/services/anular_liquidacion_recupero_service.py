...
class AnularLiquidacionRecuperoService:
    ...
    def _execute_in_transaction(
        ...
        liquidacion = self.repository.get_liquidacion_recupero_para_anular(
        ...
        operaciones = self.repository.get_operaciones_activas_liquidacion_recupero(
            id_liquidacion_recupero
        )
        if operaciones["tiene_operaciones"]:
            return AppResult.fail("LIQUIDACION_RECUPERO_TIENE_OPERACIONES")

        anulado = self.repository.anular_liquidacion_recupero(
            id_liquidacion_recupero=id_liquidacion_recupero,
            motivo=motivo,
            context=context,
        )
...
def _response(values: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "id_liquidacion_recupero": values["id_liquidacion_recupero"],
