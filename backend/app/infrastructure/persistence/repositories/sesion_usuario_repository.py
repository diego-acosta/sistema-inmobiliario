"""Persistencia central de sesiones administrativas; sin commit ni rollback."""
...
class SesionUsuarioRepository:
    ...
    def get_wall_clock_timestamp(self) -> datetime:
        ...
        # clock_timestamp avanza durante la transacción; UTC explícito conserva la
        # convención física timestamp without time zone del schema existente.
        return self.db.execute(
            text("SELECT (clock_timestamp() AT TIME ZONE 'UTC')")
        ).scalar_one()

    def insert(self, *, id_usuario: int, id_credencial_usuario: int, token_digest: str, started_at: datetime, expires_at: datetime) -> UUID:
        return UUID(str(self.db.execute(text("""
            ...
            VALUES (:user_id,:credential_id,NULL,NULL,:digest,:started,:started,NULL,
              'ACTIVA',NULL,'PASSWORD',NULL,NULL,NULL,false,:expires,NULL)
            RETURNING uid_global
        """), {"user_id": id_usuario, "credential_id": id_credencial_usuario, "digest": token_digest, "started": started_at, "expires": expires_at}).scalar_one()))
        ...
        row = self.db.execute(
            text(
                ...
                SELECT
                    ...
                    u.fecha_baja,
                    (clock_timestamp() AT TIME ZONE 'UTC') AS ahora
