...
| CU-INM-050 | Gestion de responsables de servicios trasladados | IMPLEMENTADO V1 | `POST /api/v1/asignaciones-servicio-responsable`, `GET /api/v1/asignaciones-servicio-responsable/{id}`, `GET /api/v1/asignaciones-servicio-responsable`, `PUT /api/v1/asignaciones-servicio-responsable/{id}`, `PATCH /api/v1/asignaciones-servicio-responsable/{id}/baja`; define `asignacion_servicio_responsable` por servicio + inmueble/UF + persona + vigencia + porcentaje. No materializa obligaciones financieras |
...
| CU-INM-049 | Emision de factura de servicio | FUERA DE ALCANCE | El sistema no factura servicios; solo podria registrar facturas emitidas por proveedores externos |
