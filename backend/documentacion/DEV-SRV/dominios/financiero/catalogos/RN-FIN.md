...
- aplica_a: relacion_generadora, obligacion_financiera
- origen_principal: DEV-SRV
- estado: IMPLEMENTADA como origen estructural de `relacion_generadora` y como materializacion explicita de obligacion `SERVICIO_TRASLADADO`.
...
- aplica_a: relacion_generadora, obligacion_financiera
- origen_principal: DEV-SRV
- estado: IMPLEMENTADA V1 mediante `POST /api/v1/financiero/facturas-servicio/{id_factura_servicio}/materializar`; no existe evento/consumer automatico.
...
- origen_principal: DEV-SRV
- estado: IMPLEMENTADA V1. `asignacion_servicio_responsable` esta implementada en SQL/API/backend inmobiliario y financiero la consume para crear `obligacion_obligado`.
- observaciones: inmobiliario no decide deuda ni crea obligaciones; financiero conserva ownership sobre la generacion y la composicion de la deuda.
...
### RN-FIN-080 - Trazabilidad de liquidacion de punitorio
- descripcion: Cada liquidacion positiva de `PUNITORIO` generada al registrar un pago debe persistir una fila `liquidacion_punitorio` vinculada a la obligacion, la composicion `PUNITORIO` afectada y el agrupador `uid_pago_grupo` / `codigo_pago_grupo`. La fila no modifica saldos ni reemplaza aplicaciones financieras.
- aplica_a: liquidacion_punitorio, obligacion_financiera, composicion_obligacion, movimiento_financiero
...
- aplica_a: factura_servicio, movimiento_tesoreria, egreso_proveedor_factura_servicio, liquidacion_recupero, relacion_generadora, obligacion_financiera, composicion_obligacion
- origen_principal: MODELO-FINANCIERO-FIN / SRV-INM-005 / SRV-FIN-020
...
- estado: IMPLEMENTADA V1 para registro y consulta de `comprobante_impuesto`, egreso empresa, liquidacion fiscal, consultas read-only, listado por comprobante, anulacion conservadora y bloqueo de egreso base usado.
- observaciones: `comprobante_impuesto` no genera deuda automaticamente. La modalidad financiera define si se registra solo egreso, deuda directa, pago externo informado o recupero posterior. En el backend actual estan implementadas alta y consultas del comprobante, mas egreso empresa para `EMPRESA_ASUME` y `EMPRESA_PAGA_Y_RECUPERA`; el egreso crea `movimiento_tesoreria` y `egreso_impuesto_empresa`, se puede consultar y anular logicamente, pero no crea deuda. La anulacion del egreso base se bloquea si sostiene una `liquidacion_impuesto_trasladado` activa. La deuda fiscal trasladada se crea mediante `liquidacion_impuesto_trasladado`, con `relacion_generadora`, `obligacion_financiera`, `composicion_obligacion` y `obligacion_obligado`. Las consultas read-only permiten ver detalle de liquidacion y listar liquidaciones por comprobante sin crear movimientos ni modificar saldos. Para `DIRECTO_RESPONSABLE`, `POST /api/v1/financiero/liquidaciones-impuesto-trasladado/{id}/pago-externo` registra `PAGO_EXTERNO_INFORMADO`, reduce saldo mediante `aplicacion_financiera`, exige responsable compatible, respeta idempotencia por `X-Op-Id` y no crea tesoreria, caja, egreso ni recibo. La anulacion de `liquidacion_impuesto_trasladado` es conservadora: bloquea si hay pagos, aplicaciones, punitorios u operaciones posteriores; si procede, anula liquidacion, relacion, obligacion y composiciones, y libera logicamente vinculos a egresos sin tocar `egreso_impuesto_empresa` ni `movimiento_tesoreria`.
...
- origen_principal: SRV-FIN-021
- estado: IMPLEMENTADA V1 para validacion de modalidad, egreso empresa, liquidacion fiscal, consultas y anulacion conservadora.
- observaciones: `EMPRESA_ASUME` registra egreso de tesoreria y bloquea `liquidacion_impuesto_trasladado`; no genera obligacion. `DIRECTO_RESPONSABLE` puede liquidar obligacion `IMPUESTO_TRASLADADO` sin egreso empresa y registrar pago externo informado contra esa liquidacion; el pago externo no impacta tesoreria ni recibo interno. `EMPRESA_PAGA_Y_RECUPERA` registra egreso de tesoreria y luego liquida obligacion `IMPUESTO_TRASLADADO`; requiere egreso `REGISTRADO` disponible y bloquea reutilizacion con vinculo activo. El cobro posterior usa pago normal por persona. La anulacion del egreso empresa anula el `movimiento_tesoreria` asociado y no toca deuda ni estado de cuenta; se bloquea si el egreso esta usado por una liquidacion activa.
...
- observaciones: V1 requiere aplicaciones financieras activas; si no existen, devuelve `OBLIGACION_SIN_PAGOS_APLICADOS` para que la correccion se resuelva por regeneracion. Rechaza una segunda composicion activa `AJUSTE_INDEXACION` para la misma obligacion. No implementa composiciones negativas, reemplazo de obligacion con pagos ni modificacion de pagos existentes.
...
- observaciones: V1 requiere aplicaciones financieras activas; si no existen, devuelve `OBLIGACION_SIN_PAGOS_APLICADOS` para que la correccion se resuelva por regeneracion. Aplica contra canon, conceptos morables o `AJUSTE_INDEXACION`, excluye `PUNITORIO`, no modifica pagos existentes y no crea saldo a favor persistido; si el importe supera el saldo aplicable, devuelve `remanente_no_aplicado`.
