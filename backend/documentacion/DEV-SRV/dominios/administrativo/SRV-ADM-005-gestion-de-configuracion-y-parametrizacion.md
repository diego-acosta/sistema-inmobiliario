...
Las claves contractuales se crearán por migraciones versionadas, no por alta/baja dinámica en el primer runtime. Desde #438, exposición administrativa y sensibilidad son metadata física explícita en `parametro_sistema` mediante `exponible_api_administrativa` y `es_sensible`, con política restrictiva por defecto y sin inferencias por código, tipo, nombre o valor. La editabilidad continúa pendiente de definición física; la autorización real, el cifrado o secret manager y cualquier modelo adicional de visibilidad siguen **NO CONFIRMADOS**. No se expondrán secretos en claro por API, historial, outbox ni logs. También están pendientes la prohibición física de vigencias solapadas, la idempotencia HTTP, el outbox runtime y el historial alineado al valor/contexto.
