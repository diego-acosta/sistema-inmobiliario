...
- sincronizable: sí
- genera_trazabilidad_administrativa: sí
- contrato #412: se emite exactamente una vez sólo cuando `EXECUTE` produce cambio material mediante CAS; no se emite para no-op, `REPLAY`, `CONFLICT` ni CAS mismatch. `event_type = valor_parametro_modificado`, `aggregate_type = valor_parametro`, `aggregate_id = valor_parametro.id_valor_parametro` sólo como key local interna. `outbox_event.payload` contiene `{metadata, data}`: `metadata` incluye `uid_instalacion_origen = instalacion.uid_global` y `payload_hash = SHA-256 lowercase(RFC 8785(data))`; `data` incluye `uid_global`, `codigo_parametro`, `valor_anterior` y `valor_nuevo` como decimal ASCII string, `version_anterior`, `version_registro` y `op_id`. El hash se calcula antes de construir metadata, nunca sobre el envelope autorreferencial, y es distinto del fingerprint de request #470. `data.uid_global` es la identidad portable; ningún ID numérico local se distribuye. El envelope completo se persiste en la misma transacción que CAS y receipt; `processing_metadata` no se usa para identidad/integridad de origen.
