...
- sincronizable: sí
- genera_trazabilidad_administrativa: sí
- contrato #412: se emite exactamente una vez sólo cuando `EXECUTE` produce cambio material mediante CAS; no se emite para no-op, `REPLAY`, `CONFLICT` ni CAS mismatch. `event_type = valor_parametro_modificado`, `aggregate_type = valor_parametro`, `aggregate_id = valor_parametro.id_valor_parametro`; payload distribuido allowlisted con `uid_global`, `codigo_parametro`, `valor_anterior`, `valor_nuevo`, `version_anterior`, `version_registro` y `op_id`, en la misma transacción que CAS y receipt. Los IDs numéricos locales de usuario, sucursal e instalación quedan fuera del payload; su provenance permanece en el ledger, metadata técnica local y columnas propias del outbox cuando existan.
