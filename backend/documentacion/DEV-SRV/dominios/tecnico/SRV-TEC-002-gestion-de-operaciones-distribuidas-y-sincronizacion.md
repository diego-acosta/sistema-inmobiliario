...
El claim portable usa `FOR UPDATE SKIP LOCKED` y `UPDATE ... RETURNING` dentro de una transacción Técnica corta. Cada adquisición genera un `attempt_id` UUID nuevo; dos threads, dos invocaciones o un reclaim del mismo `worker_id` nunca comparten identidad de ownership. `worker_id` se conserva sólo para observabilidad. La delivery guarda además `fence_generation` como traza monotónica. Toda transición desde una delivery portable `PROCESSING` exige el `DeliveryClaim` concreto, que contiene `attempt_id` y generación; omitirlo no habilita un modo unfenced. PostgreSQL es la única autoridad temporal mediante `clock_timestamp() AT TIME ZONE 'UTC'`: claim, reclaim, backoff y elegibilidad usan ese reloj. La expiración no revoca mágicamente al attempt: lo vuelve reemplazable. Un takeover exitoso instala otro `attempt_id`; desde ese momento el attempt anterior queda fenced aunque el `worker_id` sea el mismo. El backoff exponencial está acotado, el límite pausa únicamente la selección automática y el entry point manual reutiliza el mismo protocolo. Un resultado `BUSY` expresa contención y no consume presupuesto automático. `REJECTED` continúa terminal.
