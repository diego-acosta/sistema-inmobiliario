...
Un resultado pendiente revierte primero el efecto funcional y luego persiste solamente el estado técnico. Después de serializar por `(consumer, op_id)`, Técnico compara **todas** las otras entregas relevantes en `PENDING_DEPENDENCY`, `PROCESSING`, `PROCESSED` o `CONFLICTO`: cualquier fingerprint incompatible prevalece y termina la entrega actual en `CONFLICTO`; sólo si no hay incompatibilidad, un `PROCESSED` compatible produce replay. Entre la entrega actual y todas las compatibles aún no procesadas, el menor `id` es el líder determinista y puede ejecutar; las demás se difieren con backoff. Que una entrega compatible esté `PENDING_DEPENDENCY` o `PROCESSING` no altera esa elección después de adquirir el advisory lock, por lo que un `PROCESSING` de mayor `id` no bloquea al líder. `REJECTED` se excluye expresamente porque es un rechazo terminal de payload/imposibilidad permanente, no evidencia de un efecto aplicado ni receipt válido para replay. `(event_id, consumer)` permanece como deduplicación de entrega y no sustituye `op_id`.
