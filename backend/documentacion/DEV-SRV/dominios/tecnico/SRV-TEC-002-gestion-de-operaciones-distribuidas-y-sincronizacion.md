...
`PENDING_DEPENDENCY` es soporte transversal Técnico/Sync. `inbox_event` retiene el envelope portable, `event_id`, `consumer`, `op_id`, payload permitido, fingerprint, procedencia, intentos, elegibilidad y lease. Todo claim que declara `op_id` debe declarar también un `aggregate_uid` estable como identidad portable del target; `aggregate_id` permanece como PK local de compatibilidad y nunca la sustituye ni integra la equivalencia distribuida. Técnico parsea una sola vez `aggregate_uid` como UUID y reutiliza su representación canónica tanto en el fingerprint como en la persistencia: representaciones textuales equivalentes del mismo UUID expresan la misma identidad y producen la misma huella cuando el resto del envelope coincide. `version_registro` se valida y canonicaliza al mismo integer que persiste PostgreSQL antes de validar el envelope y calcular su fingerprint; por eso `1` y `"1"` representan la misma versión material. La política transversal default-deny se aplica en dos fronteras independientes: antes de canonicalizar, serializar o persistir un envelope nuevo y, nuevamente, después del claim visible pero antes de adquirir coordinación scoped, replay, leader election o applicator. Una fila histórica, writer viejo o inserción SQL no se considera confiable por estar en `inbox_event`: si la policy vigente rechaza evento, aggregate, payload o procedencia, termina `REJECTED` mediante transición fenced y nunca se expone al dominio. Legacy significa exclusivamente un claim payload-less sin envelope retenido, con `op_id`, payload, procedencia, `aggregate_uid` y `version_registro` nulos; sólo ese path conserva la compatibilidad sin validación portable y fingerprint nulo. El fingerprint no es provisto ni confiado al consumer: Técnico lo calcula con la canonicalización RFC 8785 común sobre `event_type`, `aggregate_type`, `aggregate_uid`, `version_registro`, payload y procedencia material. `event_id`, `consumer`, `op_id`, PK local, intentos, timestamps y lease no integran ese hash. El consumer dueño decide mediante un resultado tipado si una referencia ausente es temporal, permanente o una divergencia material; Técnico no interpreta excepciones genéricas como dependencia.

El claim usa `FOR UPDATE SKIP LOCKED` y `UPDATE ... RETURNING` dentro de una transacción Técnica corta, separada de la transacción funcional: `PROCESSING`, owner, expiración y `lease_generation` incremental se confirman y quedan visibles antes de invocar el applicator. PostgreSQL es la única autoridad temporal del lease mediante `clock_timestamp() AT TIME ZONE 'UTC'`: claim deriva `last_attempt_at` y expiración de un mismo instante DB, reclaim compara y fija elegibilidad con ese clock, y fencing exige vigencia contra ese mismo clock. `lease_duration` sigue siendo una duración configurable, nunca un timestamp calculado por el worker; el parámetro `now` del caller se conserva sólo por compatibilidad de firma y no decide elegibilidad, lease ni ownership. El processor normal reclama sólo leases vencidos de su propio `consumer`, conservando la traza; el siguiente claim incrementa la generación e invalida al owner anterior. Toda fila `PROCESSING` que tenga lease exige owner y generación esperados vigentes para cualquier transición; omitirlos nunca habilita un modo unfenced. La transición unfenced existe sólo como compatibilidad histórica para filas `PROCESSING` que realmente tienen `lease_owner IS NULL` y `lease_expires_at IS NULL`; owner y generación se aceptan juntos o se rechazan como ownership inválido. Si el fencing falla se devuelve `SYNC_INBOX_OWNERSHIP_LOST`, se revierte el savepoint funcional y el worker obsoleto no puede confirmar efecto ni estado. El backoff exponencial está acotado, el límite pausa únicamente la selección automática y el entry point manual reutiliza el mismo claim. `REJECTED` continúa terminal.
...
Cada `run_once()` posee la transacción funcional real de una `Session` dedicada al intento: puede usar savepoints internos, pero no retorna hasta ejecutar el commit exterior. Si falla cualquier mutación fenced, incluida una transición técnica posterior al applicator, o fallan applicator/commit, hace rollback completo antes de propagar; una transacción ya activa en la Session se considera parte del intento y también es cerrada por el processor, por lo que el caller no debe mezclar trabajo ajeno. `(event_id, consumer)` identifica y deduplica la delivery, mientras `(consumer, op_id)` concede el derecho de efecto independiente de cada consumer; retener una delivery mediante `claim()` no concede ese derecho. La coordinación scoped se materializa en `inbox_operation_scope` con owner, generación, expiración, fingerprint inmutable y outcome terminal; todo reclaim exige que el fingerprint solicitado coincida con el fijado al crear el scope. Se adquiere y confirma en una transacción técnica corta, usa la misma expiración DB que la delivery y no mantiene un advisory lock durante el applicator. Tras expirar, otro worker incrementa la generación y progresa sin esperar la transacción stale; efecto, transición terminal y receipt scoped comparten la transacción funcional, y el owner anterior no puede confirmarlos porque falla su fencing. No se incorpora heartbeat automático ni scheduler productivo.

Un resultado pendiente revierte primero el efecto funcional y luego persiste solamente el estado técnico. La misma transición fenced agenda `next_attempt_at` desde `clock_timestamp()` de PostgreSQL al finalizar el intento más la duración de backoff; `list_eligible()` y el claim automático comparan luego contra ese mismo reloj DB, nunca contra `now` del worker. Para una delivery scoped, el fingerprint almacenado debe existir y coincidir con la huella recomputada desde el envelope canónico antes de servir como evidencia; nunca se corrige silenciosamente. Una huella que no puede recomputarse también es inválida: la current termina `REJECTED` mediante su fencing y un sibling se excluye sin bloquear a la current. Técnico carga las otras entregas relevantes en `PENDING_DEPENDENCY`, `PROCESSING`, `PROCESSED` o `CONFLICTO`, revalida cada sibling con la policy vigente y verifica su fingerprint antes de usarlo como evidencia. Sólo siblings confiables participan en incompatibilidad, replay o leader election: los inválidos se excluyen sin mutarlos, porque el worker actual no posee su fencing; serán `REJECTED` cuando su propio `run_once()` los reclame. Entre outcomes terminales compatibles se reutiliza primero `PROCESSED` y luego `CONFLICTO`; ambos evitan reejecutar el applicator. Sin terminal compatible, entre la entrega actual y todas las compatibles y confiables aún no procesadas, el menor `id` es el líder determinista. Esta elección ocurre antes de adquirir operation ownership: sólo el líder intenta obtener el application right; un non-leader que ya lo hubiera obtenido lo libera mediante su fencing y se difiere con backoff, de modo que no puede bloquear indefinidamente el progreso lógico del líder. `REJECTED` se excluye expresamente porque es terminal y no constituye receipt válido.
