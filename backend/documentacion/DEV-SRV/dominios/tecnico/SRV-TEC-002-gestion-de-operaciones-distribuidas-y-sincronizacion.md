...
`PENDING_DEPENDENCY` es soporte transversal Técnico/Sync. `inbox_event` retiene el envelope portable, `event_id`, `consumer`, `op_id`, payload permitido, fingerprint, procedencia, intentos, elegibilidad y lease. Todo claim que declara `op_id` debe declarar también un `aggregate_uid` estable como identidad portable del target; `aggregate_id` permanece como PK local de compatibilidad y nunca la sustituye ni integra la equivalencia distribuida. Técnico parsea una sola vez `aggregate_uid` como UUID y reutiliza su representación canónica tanto en el fingerprint como en la persistencia: representaciones textuales equivalentes del mismo UUID expresan la misma identidad y producen la misma huella cuando el resto del envelope coincide. Antes de canonicalizar, serializar o persistir cualquier envelope retenido —cuando alguno de `op_id`, payload, procedencia, `aggregate_uid` o `version_registro` está presente— Técnico valida `event_type`, `aggregate_type` y payload con la política transversal default-deny, y aplica el mismo guardrail recursivo de material sensible a la procedencia; payload o provenance sensibles nunca se retienen aunque `op_id` sea nulo. Legacy significa exclusivamente un claim payload-less sin envelope retenido, con esos cinco campos nulos; sólo ese path conserva la compatibilidad sin validación portable y fingerprint nulo. El fingerprint no es provisto ni confiado al consumer: Técnico lo calcula con la canonicalización RFC 8785 común sobre `event_type`, `aggregate_type`, `aggregate_uid`, `version_registro`, payload y procedencia material. `event_id`, `consumer`, `op_id`, PK local, intentos, timestamps y lease no integran ese hash. El consumer dueño decide mediante un resultado tipado si una referencia ausente es temporal, permanente o una divergencia material; Técnico no interpreta excepciones genéricas como dependencia.
