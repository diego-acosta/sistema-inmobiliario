...
Un resultado pendiente revierte primero el efecto funcional y luego persiste solamente el estado técnico. Después de serializar por `(consumer, op_id)`, Técnico carga las otras entregas relevantes en `PENDING_DEPENDENCY`, `PROCESSING`, `PROCESSED` o `CONFLICTO` y revalida cada sibling con la policy vigente antes de usarlo como evidencia. Sólo siblings confiables participan en incompatibilidad de fingerprint, replay o leader election: los inválidos se excluyen sin mutarlos, porque el worker actual no posee su fencing; serán `REJECTED` cuando su propio `run_once()` los reclame y revalide. Entre la entrega actual y todas las compatibles y confiables aún no procesadas, el menor `id` es el líder determinista y puede ejecutar; las demás se difieren con backoff. Que una entrega confiable esté `PENDING_DEPENDENCY` o `PROCESSING` no altera esa elección después de adquirir el advisory lock, por lo que un `PROCESSING` de mayor `id` no bloquea al líder. `REJECTED` se excluye expresamente porque es un rechazo terminal de payload/imposibilidad permanente, no evidencia de un efecto aplicado ni receipt válido para replay. `(event_id, consumer)` permanece como deduplicación de entrega y no sustituye `op_id`.
