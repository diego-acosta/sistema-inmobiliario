...
Un resultado pendiente revierte primero el efecto funcional y luego persiste solamente el estado técnico. Para una delivery scoped, el fingerprint almacenado debe existir y coincidir con la huella recomputada desde el envelope canónico antes de servir como evidencia; nunca se corrige silenciosamente. La current con huella ausente o inconsistente termina `REJECTED` mediante su fencing. Después de serializar por `(consumer, op_id)`, Técnico carga las otras entregas relevantes en `PENDING_DEPENDENCY`, `PROCESSING`, `PROCESSED` o `CONFLICTO`, revalida cada sibling con la policy vigente y verifica su fingerprint antes de usarlo como evidencia. Sólo siblings confiables participan en incompatibilidad, replay o leader election: los inválidos se excluyen sin mutarlos, porque el worker actual no posee su fencing; serán `REJECTED` cuando su propio `run_once()` los reclame. Entre la entrega actual y todas las compatibles y confiables aún no procesadas, el menor `id` es el líder determinista y puede ejecutar; las demás se difieren con backoff. `REJECTED` se excluye expresamente porque es terminal y no constituye receipt válido. `(event_id, consumer)` permanece como deduplicación de entrega y no sustituye `op_id`.
