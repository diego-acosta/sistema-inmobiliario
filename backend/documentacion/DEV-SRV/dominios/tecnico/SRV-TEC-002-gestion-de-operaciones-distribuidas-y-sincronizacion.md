...
La aplicación funcional ocurre dentro de un savepoint de la `Session` compartida. Efecto, transición fenced a `PROCESSED` y receipt consumer-scoped comparten la transacción funcional; si se perdió ownership, todo el efecto local del intento obsoleto se revierte. El advisory lock `(consumer, op_id)` sólo serializa temporalmente intentos funcionales activos: no sustituye el fencing durable ni el receipt/estado `PROCESSED` que asegura unicidad durable del efecto. No se incorpora heartbeat automático: applicators largos deben usar una duración de lease adecuada; una futura renovación explícita deberá validar owner/generación, sin crear scheduler.

Un resultado pendiente revierte primero el efecto funcional y luego persiste solamente el estado técnico. Para una delivery scoped, el fingerprint almacenado debe existir y coincidir con la huella recomputada desde el envelope canónico antes de servir como evidencia; nunca se corrige silenciosamente. Una huella que no puede recomputarse también es inválida: la current termina `REJECTED` mediante su fencing y un sibling se excluye sin bloquear a la current. Después de serializar por `(consumer, op_id)`, Técnico carga las otras entregas relevantes en `PENDING_DEPENDENCY`, `PROCESSING`, `PROCESSED` o `CONFLICTO`, revalida cada sibling con la policy vigente y verifica su fingerprint antes de usarlo como evidencia. Sólo siblings confiables participan en incompatibilidad, replay o leader election: los inválidos se excluyen sin mutarlos, porque el worker actual no posee su fencing; serán `REJECTED` cuando su propio `run_once()` los reclame. Entre outcomes terminales compatibles se reutiliza primero `PROCESSED` y luego `CONFLICTO`; ambos evitan reejecutar el applicator. Sin terminal compatible, entre la entrega actual y todas las compatibles y confiables aún no procesadas, el menor `id` es el líder determinista y puede ejecutar; las demás se difieren con backoff. `REJECTED` se excluye expresamente porque es terminal y no constituye receipt válido. `(event_id, consumer)` permanece como deduplicación de entrega y no sustituye `op_id`.
