...
if not exist "%PATCH_PARAMETRIZACION_ESTRUCTURAL_FILE%" (
  echo ERROR: No existe el patch de parametrizacion estructural #409: %PATCH_PARAMETRIZACION_ESTRUCTURAL_FILE%
  pause
  exit /b 1
)
...
echo.
echo Aplicando patch parametrizacion estructural #409 en %DEV_DB%...
%PGBIN%\psql -v ON_ERROR_STOP=1 -d %DEV_DB% -f "%PATCH_PARAMETRIZACION_ESTRUCTURAL_FILE%"
if errorlevel 1 (
  echo ERROR aplicando patch parametrizacion estructural #409 en %DEV_DB%
  pause
  exit /b 1
)

echo.
echo Aplicando patch CORE-EF valor_parametro #410 en %DEV_DB%...
...
echo.
echo Aplicando patch editabilidad administrativa parametro_sistema #441 en %DEV_DB%...
...
%PGBIN%\psql -v ON_ERROR_STOP=1 -d %DEV_DB% -f "%PATCH_AUTH_CENTRAL_FILE%"
if errorlevel 1 (
    echo ERROR aplicando auth central
    exit /b 1
)
...
echo.
echo Aplicando patch parametrizacion estructural #409 en %TEST_DB%...
%PGBIN%\psql -v ON_ERROR_STOP=1 -d %TEST_DB% -f "%PATCH_PARAMETRIZACION_ESTRUCTURAL_FILE%"
if errorlevel 1 (
  echo ERROR aplicando patch parametrizacion estructural #409 en %TEST_DB%
  pause
  exit /b 1
)

echo.
echo Aplicando patch CORE-EF valor_parametro #410 en %TEST_DB%...
...
echo.
echo Aplicando patch editabilidad administrativa parametro_sistema #441 en %TEST_DB%...
...
%PGBIN%\psql -v ON_ERROR_STOP=1 -d %TEST_DB% -f "%PATCH_AUTH_CENTRAL_FILE%"
if errorlevel 1 (
    echo ERROR aplicando auth central
    exit /b 1
)
