...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "`restitucion_locativa_registrada` y materializa "
            "cierre de ocupación, disponibilidad DISPONIBLE y "
            "entrega_restitucion_inmueble en inmobiliario."
        )
    )
...
def main() -> int:
    ...
    db = SessionLocal()
    service = ConsumeRestitucionLocativaService(
        db=db,
    ...
    try:
        ...
        if not result.success:
            print(f"Error consumiendo restituciones: {', '.join(result.errors)}")
            return 1
