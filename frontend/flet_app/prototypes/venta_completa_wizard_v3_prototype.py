...
class VentaCompletaWizardV3Prototype:
    def __init__(self, page: ft.Page) -> None:
        ...
        self.preview_button: ft.Button | None = None
        ...
        return ft.Container(
            ...
            content=ft.Column(
                controls=[
                    ...
                    self._build_plan_preview_section(),
                    ft.Text(
                        "Este prototipo no calcula cronograma definitivo, indexaciones, obligaciones, divisiones por comprador ni divisiones por objeto; muestra el preview devuelto por backend cuando se simula.",
                        size=12,
                        color=ft.Colors.BLUE_GREY_600,
                    ),
                ],
        ...
        if self.state.forma_pago == "CONTADO":
            controls.extend(
                [
                    ...
                    self._build_help_card(
                        "Se enviará un único bloque CONTADO al preview backend sin id_venta y por el total de la venta.",
                        ft.Colors.BLUE_50,
                        ft.Colors.BLUE_200,
                    ),
                    self._build_plan_preview_section(),
    ...
    def _build_plan_preview_section(self) -> ft.Control:
        local_error = self._preview_local_validation_error()
        button_disabled = self.state.preview_loading or local_error is not None
        self.preview_button = ft.Button(
            "Actualizar preview" if self.state.preview_data is not None else "Simular plan",
            icon=ft.Icons.PLAY_CIRCLE_OUTLINE,
            disabled=button_disabled,
            on_click=self._load_plan_payment_preview,
        )
        controls: list[ft.Control] = [
            ft.Text("Preview Plan Pago V2", size=18, weight=ft.FontWeight.W_700),
            ft.Text(
                "POST /api/v1/ventas/plan-pago-v2/preview · PREVIEW_READLIKE sin id_venta: no crea venta ni obligaciones reales.",
                size=12,
                color=ft.Colors.BLUE_GREY_700,
            ),
            ft.Row(
        ...
        if local_error is not None:
            controls.append(self._build_help_card(local_error, ft.Colors.AMBER_50, ft.Colors.AMBER_200))
        ...
        if self.state.preview_data is not None:
            if self.state.preview_stale:
                controls.append(self._build_help_card("El preview puede estar desactualizado por cambios recientes. Volvé a simular.", ft.Colors.AMBER_50, ft.Colors.AMBER_200))
            controls.extend(self._build_plan_preview_result_controls(self.state.preview_data))
        else:
            controls.append(
                ft.Text(
                    "Todavía no hay preview backend cargado para el estado local actual del wizard.",
                    color=ft.Colors.BLUE_GREY_700,
                )
            )
        return ft.Container(
    ...
    def _load_plan_payment_preview(self, _: ft.ControlEvent | None = None) -> None:
    ...
    def _preview_local_validation_error(self) -> str | None:
        if not self.state.objetos or self._objects_total() <= Decimal("0"):
            return "Cargá objetos de venta con total mayor que 0 antes de simular el plan."
        if self.state.forma_pago not in {"CONTADO", "FINANCIADO"}:
            return "Elegí una forma de pago antes de simular el plan."
        if self.state.forma_pago == "CONTADO":
        ...
        if self.state.tiene_anticipo and not self._advance_is_valid():
            return "Completá un anticipo válido o marcá Sin anticipo antes de simular."
        if not self.state.tramos_cuotas:
            return "Para financiado, cargá al menos un tramo de cuotas antes de simular."
